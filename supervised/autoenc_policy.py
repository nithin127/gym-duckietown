...
class Model(nn.Module):
    ...
    def forward(self, image):
        ...
        enc = F.leaky_relu(self.linear2(x))
        ...
        a = F.leaky_relu(self.a_linear1(enc))
        a = self.a_linear2(a)
...
class Policy(nn.Module):
    def __init__(self, enc_size):
        ...
        # Encoding, linear combination, number of actions?
        # Just output two velocities

        self.linear = nn.Linear(enc_size, 2)
...
def eval_policy(model, policy):
    ...
    for i in range(0, 100):
        obs = make_img_var(obs).unsqueeze(0)
        rec, angle, enc = model(obs)
...
def gen_obs_actions():
    ...
    for i in range(0, 50):
        ...
        vels = np.random.uniform(low=0.3, high=0.8, size=(2,))
...
if __name__ == "__main__":
    ...
    for epoch in range(1, 1000000):
        ...
        policy = Policy(enc_size = 8)
        policy = policy.cuda()
        r = eval_policy(model, policy)

        print('r=%f, best=%f' % (r, best_reward))
